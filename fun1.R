sourceCpp("~//SpatialMaxID//stablemix//src//exp_stable.cpp")#from the stablemix package in Bopp et. al (2020)
#Find it here- https://github.com/jasa-acs/A-Hierarchical-Max-Infinitely-Divisible-Spatial-Model-for-Extreme-Precipitation/tree/master/project_code
  ...
  function(n_iter,
    ...
    for(i in 1:n_iter){
      ...
      if(!fix_theta0){
        ...
        if(theta_prop > 0 & !is.infinite(theta_prop)){
      ...
      if("mu"%in%mar_gp_which){
        ...
        for(j in 1:nclus[["mu"]]){
          ...
          sub <- (clusters[["mu"]] ==j)
          if(any(sub)){
            mu_gp_prop_uncons=mu_gp_cur
            Csub <- Cmat[sub, sub]
            Lt <- chol(Csub)
            white <- mu_gp_cur[sub]%*%solve(Lt)
            white_shifted <- white + rnorm(sum(sub), mean = 0, sd = sqrt(mu_gp_prop_var[pos, j]))
            mu_gp_prop_uncons[sub]=white_shifted%*%Lt
            mu_gp_prop_cons=mu_gp_prop_uncons-Cmat%*%Xloc[1:nloc,c(1:(ploc-2))]%*%solve(t(Xloc[1:nloc,c(1:(ploc-2))])%*%Cmat%*%Xloc[1:nloc,c(1:(ploc-2))])%*%t(Xloc[1:nloc,c(1:(ploc-2))])%*%mu_gp_prop_uncons
            mu_gp_prop <- mu_gp_prop_cons
      ...
      if("sigma"%in%mar_gp_which){
        ...
        for(j in 1:nclus[["sigma"]]){
          ...
          if(any(sub)){
            sigma_gp_prop_uncons=sigma_gp_cur
            Csub <- Cmat[sub, sub]
            Lt <- chol(Csub)
            white <- sigma_gp_cur[sub]%*%solve(Lt)
            white_shifted <- white + rnorm(sum(sub), mean = 0, sd = sqrt(sigma_gp_prop_var[pos, j]))
            sigma_gp_prop_uncons[sub]=white_shifted%*%Lt
            sigma_gp_prop_cons=sigma_gp_prop_uncons-Cmat%*%Xscale[1:nloc,c(1:(pscale-2))]%*%solve(t(Xscale[1:nloc,c(1:(pscale-2))])%*%Cmat%*%Xscale[1:nloc,c(1:(pscale-2))])%*%t(Xscale[1:nloc,c(1:(pscale-2))])%*%sigma_gp_prop_uncons
            sigma_gp_prop <- sigma_gp_prop_cons
      ...
      if(i%%thin_int ==0){
        ...
        save(smp_alpha,smp_theta,smp_gscl,smp_gvar,smp_locpar,smp_scalepar,smp_shapepar,file="~//work//SpatialMaxID//Results11//smp_par.Rdata")
        if("mu"%in%mar_gp_which){
          smp_mu_gp[smp_loc,] <- mu_gp_cur
          smp_mu_gvar[smp_loc] <- mu_gvar_cur
          smp_mu_gscl[smp_loc] <- mu_gscl_cur
          save(smp_mu_gp,smp_mu_gvar,smp_mu_gscl, file="~//work//SpatialMaxID//Results11//smp_mu.Rdata"  )
        }
        else{
        ...
        if(is.null(select_lat)){
          ...
          save(smp_lB, file="~//work//SpatialMaxID//Results11//smp_lB.Rdata")
          save(smp_lK, file="~//work//SpatialMaxID//Results11//smp_lK.Rdata")
        }
        else{
    ...
    return(list(smp_par = smp_mat,
                ...
                final_mu_prop_var = mu_gp_prop_var[pos,],
  ...
  if ((mpar_gscl > 0) & (mpar_gvar > 0) & all(sigma > 0)) {
    if (is.null(rng_max)) {
      llikM(lz, y, lA, lK, alpha, theta, mu, sigma, xi) +
        ...
        dnorm(mpar_gscl, sd = 10, log = TRUE)
    }
    else{
      llikM(lz, y, lA, lK, alpha, theta, mu, sigma, xi) +
        llgp(gp, s, mpar_gvar, mpar_gscl) +
